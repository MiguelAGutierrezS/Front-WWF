...
export const ReportConfigModal = () => {
  ...
  const [localActiveCharts, setLocalActiveCharts] = useState(reportFilters.activeCharts || ['biodiversity', 'seasonal', 'accumulation', 'activity', 'occupancy', 'temperature', 'prey', 'rai', 'trophic', 'timeline', 'frequency', 'pie']);
    ...
    <FloatingPanel className="w-full max-w-4xl max-h-[90vh] mx-auto mt-[5vh] p-8 bg-black/95 backdrop-blur-3xl flex flex-col rounded-3xl border border-white/10 shadow-2xl relative">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-4 space-y-8">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { id: 'accumulation', label: 'Curva de Acumulación' },
              ...
              <label key={chart.id} className={`flex items-center p-4 rounded-xl border cursor-pointer transition-all duration-300 hover:scale-[1.03] ${localActiveCharts.includes(chart.id) ? 'bg-white border-white text-black' : 'bg-white/5 border-white/10 text-gray-400 hover:bg-[#1a1a1a]'}`}>
                <input 
                  type="checkbox" 
                  className="hidden" 
        ...
        <section>
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1 bg-white/5 p-4 rounded-xl border border-white/10">
              <label className="block text-xs font-semibold text-gray-400 uppercase mb-2">Fecha Fin <span className="text-red-400">*</span></label>
              <input 
                type="date" 
                value={localEndDate}
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Settings2 className="w-5 h-5 text-gray-200" />
            ...
            <button 
              ...
              className="text-sm text-gray-300 hover:text-white font-semibold cursor-pointer"
            >
        ...
        <section>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-xl font-bold text-white">4. Periodo del Día</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {['Mañana', 'Tarde', 'Noche'].map(period => (
              <label key={period} className={`flex items-center p-4 rounded-xl border cursor-pointer transition-all duration-300 hover:scale-[1.03] ${localActivePeriods.includes(period) ? 'bg-white border-white text-black' : 'bg-white/5 border-white/10 text-gray-400 hover:bg-[#1a1a1a]'}`}>
                <input 
                  type="checkbox" 
                  className="hidden" 
        ...
        <section>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-xl font-bold text-white">5. Rango de Temperatura (°C)</h3>
          </div>
          <div className="flex gap-4">
            <div className="flex-1 bg-white/5 p-4 rounded-xl border border-white/10">
              <label className="block text-xs font-semibold text-gray-400 uppercase mb-2">Temp Mínima (°C)</label>
              <input 
                type="number" 
                placeholder="Ej. 20"
                value={localTempMin}
            ...
            <div className="flex-1 bg-white/5 p-4 rounded-xl border border-white/10">
              <label className="block text-xs font-semibold text-gray-400 uppercase mb-2">Temp Máxima (°C)</label>
              <input 
                type="number" 
                placeholder="Ej. 35"
                value={localTempMax}
      ...
      <div className="mt-8 pt-6 border-t border-white/10 flex justify-between items-center gap-4">
        <div className="flex-1">
          {errorMsg && <p className="text-red-400 font-bold animate-in fade-in slide-in-from-bottom-2">{errorMsg}</p>}
        </div>
        <div className="flex gap-4">
          <button 
